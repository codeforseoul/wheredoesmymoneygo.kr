...
OpenSpending.DailyBread = function (elem, opts) {
  ...
  this.init = function () {
    this.setSalary(self.opts.defaultsalary || 30000000); // default starting salary
  ...
  this.getTaxVal = function () {
    var rq = $.getJSON(TAXMAN_URL + '/'+(self.opts.country || 'ko')+'?callback=?', $.extend({
      year: 2013,
      indirects: true,
  ...
  this.drawTotals = function () {
    $('#db-salary-' + self.opts.region + ' p').text(formatCurrency(self.salaryVal, 0, self.opts.symbol))
    $('#db-tax-' + self.opts.region + ' p').text(formatCurrency(self.taxVal, 0, self.opts.symbol))
  }
